...
@section('title', $isFrench ? 'Nouvelle Leçon' : 'New Lesson')
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            <nav class="flex items-center gap-2 text-sm mb-4 flex-wrap" aria-label="Breadcrumb">
                ...
                <i class="fas fa-chevron-right text-gray-400 text-xs"></i>
                <span class="text-amber-600 font-semibold">{{ $isFrench ? 'Nouvelle' : 'New' }}</span>
            </nav>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <form action="{{ route('admin.auto-ecole.cours.lecons.store', [$module->id, $chapitre->id]) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="contenu_texte" 
                                  ...
                                  required
                                  rows="8"
                                  class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-200 @error('contenu_texte') border-red-500 @enderror">{{ old('contenu_texte') }}</textarea>
                        @error('contenu_texte')
                            <p class="mt-2 text-sm text-red-600 flex items-center">
                                <i class="fas fa-exclamation-circle mr-1"></i>
                                {{ $message }}
                    ...
                    <div>
                        <label for="image" class="block text-sm font-semibold text-gray-700 mb-2">
                            ...
                            {{ $isFrench ? 'Image (optionnelle)' : 'Image (optional)' }}
                        </label>
                        <input type="file" 
                               name="image" 
                               id="image" 
                ...
                <div class="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-gray-200">
                    <button type="submit" 
                        ...
                        {{ $isFrench ? 'Créer la leçon' : 'Create lesson' }}
                    </button>
