...
@section('title', $isFrench ? 'Modifier le Chapitre' : 'Edit Chapter')
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl md:text-4xl font-bold text-gray-900">
                ...
                {{ $isFrench ? 'Modifier le Chapitre' : 'Edit Chapter' }}
            </h1>
        </div>

        <!-- Formulaire -->
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <form action="{{ route('admin.auto-ecole.cours.chapitres.update', [$module->id, $chapitre->id]) }}" method="POST" class="p-6 md:p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" 
                                  id="description" 
                                  rows="4"
                                  class="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-amber-500 focus:ring-2 focus:ring-amber-200 transition-all duration-200 @error('description') border-red-500 @enderror">{{ old('description', $chapitre->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600 flex items-center">
                                <i class="fas fa-exclamation-circle mr-1"></i>
                                {{ $message }}
