...
@section('title', 'Nouvel Utilisateur Auto-École')
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600 mt-1">Ajouter un nouveau candidat à la formation</p>
        </div>
    ...
    <form action="{{ route('admin.auto-ecole.users.store') }}" method="POST" class="space-y-6">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="adresse" rows="2" 
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('adresse') }}</textarea>
                    @error('adresse')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="numero_telephone" value="{{ old('numero_telephone') }}" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="centre_examen_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach($centresExamen as $centre)
                            <option value="{{ $centre->id }}" {{ old('centre_examen_id') == $centre->id ? 'selected' : '' }}>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Parrain (optionnel)</label>
                    <input type="text" name="parrain_telephone" value="{{ old('parrain_telephone') }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                           placeholder="Téléphone du parrain">
                    @error('parrain_telephone')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        @foreach($joursPratique as $jour)
                            <label class="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" name="jours_pratique[]" value="{{ $jour->id }}"
                                       {{ in_array($jour->id, old('jours_pratique', [])) ? 'checked' : '' }}
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="paiement_x" value="1" {{ old('paiement_x') ? 'checked' : '' }}
                           class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                    <span class="text-gray-700">Paiement X effectué</span>
                </label>

                <label class="flex items-center space-x-3 cursor-pointer">
                    <input type="checkbox" name="paiement_y" value="1" {{ old('paiement_y') ? 'checked' : '' }}
                           class="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
